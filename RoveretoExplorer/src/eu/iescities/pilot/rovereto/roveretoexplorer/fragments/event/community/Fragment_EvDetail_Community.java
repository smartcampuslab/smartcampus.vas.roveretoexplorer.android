...
import eu.iescities.pilot.rovereto.roveretoexplorer.custom.RatingHelper;
import eu.iescities.pilot.rovereto.roveretoexplorer.custom.RatingHelper.RatingHandler;
import eu.iescities.pilot.rovereto.roveretoexplorer.custom.ReviewHelper;
...
public class Fragment_EvDetail_Community extends Fragment implements RefreshComments{
	...
	public void onStart() {
		...
		mEventId = bundle.getString(Utils.ARG_EVENT_ID);
		// try {
		// if (mEventId!=null)
...
//			adapter.notifyDataSetChanged();
	...
	private void set_attending_interaction(View header2) {
		...
		if (getEvent().getCommunityData().getAttending() == null
		...
		mAttending_my.setOnCheckedChangeListener(new OnCheckedChangeListener() {
			...
			public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
				...
				{
					new SCAsyncTask<Boolean, Void, ExplorerObject>(getActivity(), new AttendProcessor(getActivity(),
							buttonView)).execute(getEvent().getCommunityData().getAttending() == null
							|| getEvent().getCommunityData().getAttending().isEmpty());
				}
			}
		});
	...
	private class AttendProcessor extends AbstractAsyncTaskProcessor<Boolean, ExplorerObject> {
		...
		public void handleResult(ExplorerObject result) {
			...
			if (getActivity() != null)
				if (mEvent.getCommunityData().getAttending() == null
						|| mEvent.getCommunityData().getAttending().isEmpty()) {
